...
def save_patches(image_path, label_path, patch_size, patch_save_path, max_samples=None):
    ...
    for id in tqdm(ids):
        ...
        for i in range(len(im_patches)):
            ...
            np.save(os.path.join(im_patch_path, id + f'_{i}'), im_patch)
            np.save(os.path.join(lab_patch_path, id + f'_{i}'), lab_patch)
        # np.save(os.path.join(im_patch_path, id), im_patches)
        # np.save(os.path.join(lab_patch_path, id), label_patches)


def load_patches_data_split(patch_path, patch_size, train_prop, max_samples=None):
    ...
    image_ids = [f.split('_')[0] for f in patch_files]
    ...
    if max_samples:
        ...
        id_train = image_ids[:train_prop]
        id_test = image_ids[train_prop:]
    ...
    # load one image so we can figure out dimensions (patch size etc.)
    image = np.load(os.path.join(im_patch_path, id_train[0]))
    X_train = np.zeros((len(id_train), image.shape[0], image.shape[1], image.shape[2]))
    y_train = np.zeros((len(id_train), image.shape[0], image.shape[1]))
    ...
    print(f"Loading {len(id_train)} training patches...")
    for i, id in tqdm(enumerate(id_train), total=len(id_train)):
    ...
    X_test = np.zeros((len(id_test), patch_size, patch_size, 3))
    y_test = np.zeros((len(id_test), patch_size, patch_size))
    print(f"Loading {len(id_test)} testing patches...")
    for i, id in tqdm(enumerate(id_test)):
        image = np.load(os.path.join(im_patch_path, id))
        label = np.load(os.path.join(lab_patch_path, id))
...
def main(image_path, labels_path, model_path, output_path):
    ...
    X_train, y_train, X_test, y_test = load_patches_data_split(PATCH_PATH, patchsize, 0.8, max_samples=max_samples)
