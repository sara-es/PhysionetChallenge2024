...
def read_config_file(config_file: yaml) -> dict[str, str]:
...
def load_recording(recording_file: str, header=None, key='val'):
...
def convert_bounding_boxes_to_dict(lead_bboxes, text_bboxes, labels, startTimeList=None, endTimeList=None,
    ...
    for i in range(len(labels)):
        ...
        current_lead_ds["plotted_pixels"] = [[plotted_pixels_dict[i][j][0], plotted_pixels_dict[i][j][1]] for j in
                                             range(len(plotted_pixels_dict[i]))]
