"""
WARNING: there is currently no reason to use this script - we have no models that are trained on images. 
If you are aiming to test image cleaning using filters and ecg-miner, please use `val_digitization.py` instead.
...
def five_fold_cv(data_folder=None, multilabel_cv=True, n_splits=5, train_models=True):
    ...
    for i, (train_idx, test_idx) in enumerate(kfold.split(indeces, labels)):
        ...
        run(data_folder, test_records, output_folder, team_digit_model, None,
            allow_failures=False, verbose=True)
...
def parse_args():
    ...
    parser.add_argument('-m', '--multilabel_cv', default=True, type=bool)
    parser.add_argument('-n', '--n_splits', default=5, type=int)
    parser.add_argument('-t', '--train_models', default=True, type=bool)
