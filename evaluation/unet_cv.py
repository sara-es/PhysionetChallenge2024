...
def prepare_data_split(image_path, label_path, patch_size, train_prop):
    id = sorted(os.listdir(label_path))
    print(id)
    id = shuffle(id)
    ...
    train_prop = int(len(id) * 0.002)
    test_prop = int(len(id) * 0.0022)
    id_train = id[:train_prop]
    id_test = id[train_prop:test_prop]
...
def main(image_path, labels_path, model_path, output_path):
    ...
    PATH_UNET = model_path # this is where the model will be saved
    CHK_PATH_UNET = model_path + '_checkpoint' # this is where the model checkpoints will be saved, used with early stopping
    LOSS_PATH = model_path + '_losses' # this is where the loss values will be saved, used with early stopping
    ...
    # could bypass this by creating patches in dataloaders
    max_samples = False #15000 
...
model_path = 'G:\\PhysionetChallenge2024\\model\\UNET_256'
